    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-xl border border-[#e5e7eb] bg-white p-3">
        ...
        <select
          ...
          className="rounded-lg border border-[#e5e7eb] bg-slate-50 px-2 py-1.5 text-xs text-slate-700"
        >
        ...
        <input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="rounded-lg border border-[#e5e7eb] bg-slate-50 px-2 py-1.5 text-xs text-slate-700" />
        <span className="text-xs text-slate-400">~</span>
        <input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="rounded-lg border border-[#e5e7eb] bg-slate-50 px-2 py-1.5 text-xs text-slate-700" />
      </div>
          ...
          <tbody className="divide-y divide-[#f1f5f9]">
            ...
            {filtered.map((i) => (
              <tr key={i.id} className={i.result === "nonconforming" ? "bg-red-500/10" : i.result === "recheck" ? "bg-orange-500/10" : ""}>
                ...
                <td className="whitespace-nowrap px-3 py-2 font-medium text-slate-600">{zoneName(i.zoneId)}</td>

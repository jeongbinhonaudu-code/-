  ...
  return (
    <div className="mx-auto max-w-[1200px] px-3 py-4 sm:px-4 sm:py-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h1 className="text-xl font-bold text-slate-100">생산 시뮬레이션</h1>
          <p className="mt-1 text-sm text-slate-400">조건을 조절해 예상 월 생산수량 변화를 가늠해봅니다.</p>
        </div>
        <button
          onClick={() => setInputs(SIMULATION_DEFAULTS)}
          className="flex items-center gap-1.5 rounded-lg border border-white/15 bg-[#101c33] px-3 py-2 text-xs font-semibold text-slate-300 hover:bg-white/5"
        >
          <RotateCcw size={14} /> 현재값 초기화
        </button>
      </div>

      <div className="rounded-xl border border-orange-500/30 bg-orange-500/10 px-4 py-3 text-xs font-semibold text-orange-300">
        ⚠ 가정 실험입니다. 실제 원인을 확정하거나 생산량을 보장하는 예측이 아닙니다.
      </div>

      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="space-y-5 rounded-xl border border-white/10 bg-[#101c33] p-5">
          <Slider
            label="월 가동일"
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-sky-500/30 bg-gradient-to-br from-[#132a4d] to-[#0d1c38] p-5 text-white shadow-[0_0_0_1px_rgba(56,189,248,0.05)]">
          ...
          <div className="rounded-xl border border-white/10 bg-[#101c33] p-5">
            ...
            <div className="mt-1.5 flex items-center justify-between text-xs">
              <span className="font-bold text-slate-200">{result.progressTowardPastPct}%</span>
              <span className={result.reachedPastTarget ? "font-semibold text-emerald-400" : "text-slate-400"}>
          ...
          <div className="rounded-xl border border-white/10 bg-[#101c33] p-5">
            ...
            <p className="text-lg font-bold text-slate-200">{SIMULATION_FACTOR_LABELS[result.mostInfluentialFactor]}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      <div className="mb-1.5 flex items-center justify-between">
        ...
        <span className="rounded-md bg-white/5 px-2 py-0.5 text-sm font-bold text-slate-100">
          {value}
          {unit}
        </span>
      </div>

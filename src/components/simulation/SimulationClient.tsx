  ...
  return (
    <div className="mx-auto max-w-[1200px] px-3 py-4 sm:px-4 sm:py-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h1 className="text-xl font-bold text-[#111827]">생산 시뮬레이션</h1>
          <p className="mt-1 text-sm text-slate-400">조건을 조절해 예상 월 생산수량 변화를 가늠해봅니다.</p>
        </div>
        <button
          onClick={() => setInputs(SIMULATION_DEFAULTS)}
          className="flex items-center gap-1.5 rounded-lg border border-[#e5e7eb] bg-white px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50"
        >
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-blue-200 bg-gradient-to-br from-blue-50 to-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
            <p className="text-xs text-slate-500">예상 월 생산수량 (가정 결과)</p>
            <p className="mt-1 text-4xl font-black text-[#111827]">{result.expectedQuantity.toLocaleString()}개</p>
            <div className="mt-3 grid grid-cols-2 gap-3 text-xs">
              <div className="rounded-lg bg-white px-3 py-2 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
                <p className="text-slate-500">현재({BASE_MONTHLY_QUANTITY.toLocaleString()}개) 대비</p>
                <p className={"flex items-center gap-1 font-bold " + (result.vsCurrentDiff >= 0 ? "text-emerald-600" : "text-red-600")}>
              ...
              <div className="rounded-lg bg-white px-3 py-2 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
                <p className="text-slate-500">과거 기준(4,000개) 대비</p>
                <p className={"flex items-center gap-1 font-bold " + (result.vsPastDiff >= 0 ? "text-emerald-600" : "text-red-600")}>
          ...
          <div className="rounded-xl border border-[#e5e7eb] bg-white p-5">
            ...
            <div className="mt-1.5 flex items-center justify-between text-xs">
              ...
              <span className={result.reachedPastTarget ? "font-semibold text-emerald-600" : "text-slate-500"}>
          ...
          <div className="rounded-xl border border-[#e5e7eb] bg-white p-5">
            <p className="mb-1 flex items-center gap-1.5 text-xs font-semibold text-slate-400">
              <AlertTriangle size={13} className="text-orange-400" /> 가장 영향이 큰 조건 (기본값 대비 편차 기준)
            </p>
            <p className="text-lg font-bold text-slate-700">{SIMULATION_FACTOR_LABELS[result.mostInfluentialFactor]}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      <div className="mb-1.5 flex items-center justify-between">
        ...
        <span className="rounded-md bg-slate-50 px-2 py-0.5 text-sm font-bold text-[#111827]">
          {value}
          {unit}
        </span>
      </div>
